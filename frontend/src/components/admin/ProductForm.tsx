...
import { Category, Description, EditLocation, Image, Phone, Title } from "@mui/icons-material";
import { Box, FormHelperText, Grid, InputAdornment, MenuItem, Typography } from "@mui/material";
...
const ProductForm: React.FC<ProductFormProps> = ({ editMode = false, productData = {} }) => {
  const [name, setName] = useState(productData.name || "");
  const [description, setDescription] = useState(productData.description || "");
  const [price, setPrice] = useState(productData.price || "");
  const [category, setCategory] = useState(productData.category || "");
  const [state, setState] = useState(productData.state || "");
  const [sellerPhone, setSellerPhone] = useState(productData.sellerPhone?.replace(/\D/g, "") || "");
  ...
  const handlePriceChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    let value = event.target.value
      .replace(/[^0-9.,]/g, "") // Deixa só números, vírgula e ponto
      .replace(/,/g, "."); // Converte vírgula para ponto

    // Permite apenas um ponto (para decimal)
    const parts = value.split(".");
    if (parts.length > 2) {
      value = parts[0] + "." + parts[1];
    }

    // Limita a 2 casas após o ponto
    if (parts[1]?.length > 2) {
  ...
  const validateForm = () => {
    ...
    const rawPrice = price.replace(/[^\d,]/g, ""); // Remove "R$" e outros caracteres não numéricos
    if (!rawPrice) {
      newErrors.price = "Preço é obrigatório";
    } else if (
      isNaN(Number(rawPrice.replace(",", "."))) ||
      Number(rawPrice.replace(",", ".")) <= 0
    ) {
  ...
  return (
    <Card sx={{ maxWidth: 800, mx: "auto", p: 4, mt: 4 }}>
      <Typography variant="h5" component="h1" fontWeight="bold" mb={4} textAlign="center">
        ...
        <Grid container spacing={3}>
              ...
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">
                    <Title sx={{ color: errors.name ? "error.main" : "#2e7d32" }} />
                  </InputAdornment>
                ),
              }}
            />
          ...
          <Grid item xs={12}>
            <StyledTextField
              ...
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">
                    <Description sx={{ color: errors.description ? "error.main" : "#2e7d32" }} />
                  </InputAdornment>
                ),
              }}
            />
          ...
          <Grid item xs={12} sm={6}>
            <StyledTextField
              ...
              onBlur={(e) => setPrice(Number(price.replace(",", ".")).toFixed(2).replace(".", ","))}
              InputProps={{
          ...
          <Grid item xs={12} sm={6}>
            <StyledTextField
              ...
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">
                    <EditLocation sx={{ color: errors.state ? "error.main" : "#2e7d32" }} />
                  </InputAdornment>
                ),
              }}
            >
          ...
          <Grid item xs={12}>
            <StyledTextField
              ...
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">
                    <Category sx={{ color: errors.category ? "error.main" : "#2e7d32" }} />
                  </InputAdornment>
                ),
              }}
            >
          ...
          <Grid item xs={12}>
            <StyledTextField
              ...
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">
                    <Phone sx={{ color: errors.sellerPhone ? "error.main" : "#2e7d32" }} />
                  </InputAdornment>
                ),
              }}
            ...
            />
            <FormHelperText>
              Insira DDD + telefone, nós colocamos o formato correto. Ex: 11912345678
            </FormHelperText>
          </Grid>
          ...
          <Grid item xs={12}>
            ...
            />
            <FormHelperText>
              Deixe em branco para usar uma imagem padrão. Recomendamos imagens de 400x300 pixels.
            </FormHelperText>
          </Grid>
